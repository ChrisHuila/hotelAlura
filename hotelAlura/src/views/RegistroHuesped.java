...
public class RegistroHuesped extends JFrame {
	...
	public Object editarHuesped[];
	public static boolean borrarCamposHuesped;
	...
	public RegistroHuesped() {
		...
		this.idReserva = ReservasView.idReserva;  
		
		if(!borrarCamposHuesped) {			
			this.editarHuesped = Busqueda.editarHuesped;		
		}
...
//		Obtiene la nacionalidad
		setTxtNacionalidad(txtNacionalidad);
...
//				Limita la cantidad de caracteres max 15
...
//		Carga los datos para editarlos
		if(this.editarHuesped != null) {//			
			txtNombre.setText((String) editarHuesped[1]);
			txtApellido.setText((String) editarHuesped[2]);
			txtFechaN.setDate(java.sql.Date.valueOf((String) editarHuesped[6]));
			txtNacionalidad.setSelectedItem(editarHuesped[3]);
			txtTelefono.setText((String) editarHuesped[4]);
			txtNreserva.setText((String) editarHuesped[5]);
		}
...
//				Valida que no haya campos vacios
				if (getTxtNombre().getText().length() != 0 &&
					getTxtApellido().getText().length() != 0 && 
					getTxtTelefono().getText().length() != 0 && 
					getTxtFechaN().getDate() != null  && editarHuesped == null
					) {
//					guarda
					guardarHuesped();
					
//					Hace visible el mensaje de exito
					Exito mensajeExito = new Exito();
					mensajeExito.setVisible(true);
					dispose();
				}else if (editarHuesped != null) {
//					edita
					editarHuesped();
...
public void editarHuesped() {
		...
		String dateNacimiento = dcn.format(getTxtFechaN().getDate());
		
		Huesped huesped = new Huesped(Integer.parseInt((String) editarHuesped[0]),getTxtNombre().getText(), getTxtApellido().getText(),
				getTxtNacionalidad().getSelectedItem().toString(), Long.parseLong(getTxtTelefono().getText().trim()),
				Integer.parseInt(getTxtNreserva().getText()),java.sql.Date.valueOf(dateNacimiento));
		
		controllerHuesped.actualizarHuesped(huesped);
	...
	public void guardarHuesped() {		
		...
		Huesped huesped = new Huesped(getTxtNombre().getText(), getTxtApellido().getText(),
				getTxtNacionalidad().getSelectedItem().toString(), Long.parseLong(getTxtTelefono().getText().trim()),
				Integer.parseInt(getTxtNreserva().getText()),java.sql.Date.valueOf(dateNacimiento) );
		
		controllerHuesped.guardar(huesped);
